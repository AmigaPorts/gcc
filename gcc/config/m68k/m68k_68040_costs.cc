...
/* ============================================
   68040 KOSTEN-MAKROS (Speed / Size)
   ============================================ */
#define COST_REG                    (speed ? 1  : 5)
#define COST_MEM_PLUS_REG_DISP      (speed ? 0  : 1)
#define COST_MEM_PLUS_REG_REG       (speed ? 3  : 0)
#define COST_MEM_OTHER              (speed ? 6  : 1)
#define COST_CONST_INT_Q            (speed ? 0  : 20)
#define COST_CONST_INT_W            (speed ? 0  : 1)
#define COST_CONST_INT_L            (speed ? 4  : 0)
#define COST_CONST_DOUBLE           (speed ? 4  : 9)
#define COST_SYMBOL                 (speed ? 0  : 12)
#define COST_PLUS_REG_REG           (speed ? 2  : 5)
#define COST_PLUS_REG_CONSTQ        (speed ? 0  : 20)
#define COST_PLUS_REG_CONSTW        (speed ? 2  : 0)
#define COST_PLUS_REG_CONSTL        (speed ? 3  : 1)
#define COST_PLUS_OTHER             (speed ? 12 : 12)
#define COST_LOGIC_REG_CONST        (speed ? 4  : 8)
#define COST_SHIFT_CONST            (speed ? 2  : 2)
#define COST_SHIFT_CONST_W          (speed ? 3  : 0)
#define COST_BRANCH                 (speed ? 3  : 0)
#define COST_SET_REG_REG            (speed ? 2  : 1)
#define COST_CALL_OTHER             (speed ? 4  : 11)

/* ============================================
   68040 MULT / DIV KOSTEN
   ============================================ */
#define COST_MUL_L                  20
#define COST_MUL_W                  14
#define COST_DIV_L                  38
#define COST_DIV_W                  44
#define COST_ADD_L                  1
#define COST_SUB_L                  1
#define COST_MOVE_L                 1
#define COST_MOVEQ                  1
#define COST_LSL_SHIFT(shift)       (2 + (shift) * 0)

/* ============================================
   m68k_68040_costs_intern
   ============================================ */
static bool
m68k_68040_costs_intern (rtx x,
                         machine_mode mode,
                         ...
                         bool speed)
{
    ...
    case SET:
    {
        ...
        if (!m68k_68040_costs_intern(dest, mode, code, 0, total, speed))
            return false;
        if (!m68k_68040_costs_intern(src, mode, code, 1, &total2, speed))
            return false;
        *total += total2;
...
m68k_68040_costs (rtx x,
                  ...
                  bool speed)
{
    return m68k_68040_costs_intern(x, mode, outer_code, opno, total, speed);
}
